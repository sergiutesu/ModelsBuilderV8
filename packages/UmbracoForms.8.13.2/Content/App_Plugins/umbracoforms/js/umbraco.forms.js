angular.module("umbraco").controller("UmbracoForms.EditTypes.CheckboxListController",
  function ($scope) {
    ...
    vm.isChecked = function (preValue) {
      return vm.value.includes(preValue.value);
    };
...
angular.module("umbraco").controller("UmbracoForms.Editors.Form.EntriesController",
  function ($scope, $routeParams, recordResource, formResource, editorService, userService, securityResource, notificationsService, navigationService, overlayService, localizationService, utilityService, preValueSourceResource, formService) {
    ...
    vm.getWorkflowSummaryTitle = getWorkflowSummaryTitle;
    vm.fieldValueOrPrevalueCaption = fieldValueOrPrevalueCaption;
              ...
              recordResource.getRecordSetActions().then(function (response) {
                ...
                for (var fi = 0; fi < formFields.length; fi++) {
                  ...
                  if ((!field.preValues || field.preValues.length == 0) &&
                    ...
                    // Need to create a closure here to ensure field.id is correct for each item in the loop.
                    (function (fieldId) {
    ...
    function getPrevaluesForEditing(fieldId) {
      ...
      return formField.parsedPreValues && formField.parsedPreValues.length > 0
        ? formField.parsedPreValues
        : formField.preValues;
    }
    ...
    function fieldValueOrPrevalueCaption(value, fieldId) {
      ...
      if (formField.preValues && formField.preValues.length > 0) {
        ...
        for (var i = 0; i < formField.preValues.length; i++) {
          var prevalue = getPrevalueForField(formField, i);
          if (prevalue.value == value) {
            matchedWithPrevalue = true;
            result = (prevalue.caption && prevalue.caption.length > 0)
              ? prevalue.caption
              : value;
          }
        }
        ...
        if (!matchedWithPrevalue && value.includes(prevalueValueSeparator)) {
          ...
          for (var i = 0; i < valueParts.length; i++) {
            ...
            for (var j = 0; j < formField.preValues.length; j++) {
              var prevalue = getPrevalueForField(formField, j);
              if (prevalue.value == valueParts[i]) {
                resultParts.push((prevalue.caption && prevalue.caption.length > 0)
                  ? prevalue.caption
                  : valueParts[i]);
                matchedWithPrevalue = true;
                break;
            ...
            if (!matchedWithPrevalue) {
              resultParts.push(valueParts[i]); // Didn't find a prevalue match for this part of the record value, so use the value part instead.
            }
          }
    ...
    function getPrevalueForField(formField, index) {
      return formField.parsedPreValues && formField.parsedPreValues.length > 0
        ? formField.parsedPreValues[index]
        : formField.preValues[index];
    }
    ...
    function getWorkflowSummaryTitle(field) {
...
(function () {
  ...
  function FormPickerOverlayController($scope, formPickerResource) {
    ...
    vm.allowFolderSelection = Umbraco.Sys.ServerVariables.umbracoPlugins.forms.storeUmbracoFormsInDb &&
      (!$scope.model.allowedForms || $scope.model.allowedForms.length === 0);
    ...
    function treeNodeExpandedHandler(args) {
      fixTreeExpandButtons();
    }

    function fixTreeExpandButtons() {
...
(function () {
  ...
  function WorkflowSettingsOverlayController($scope, workflowResource, localizationService, providerLocalizationHelper, formService) {
...
(function () {
  ...
  function FormsEntryDetail(userService, currentUserResource, editorService, recordResource, overlayService, localizationService, notificationsService) {
    ...
    function link(scope, el, attr, ctrl) {
      ...
      var loadWorkflowAuditTrail = function () {
